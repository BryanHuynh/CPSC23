    ...
    public void constructRoom(int x, int y, char[][] map){
        ...
        for(int j = 0; j < height; j++) {
            for (int i = 0; i < length; i++) {
                if(j == 0 && i == 0){ map[j + y][i + x] = '#'; continue;}
                if(j == height - 1 && i == 0) { map[j + y][i + x] = '#'; continue;}
                if(j == height - 1 && i == length - 1) { map[j + y][i + x] = '#'; continue;}
                if(j == 0 && i == length - 1) { map[j + y][i + x] = '#'; continue;}
                map[j + y][i + x] = 'W';
            }
        }
